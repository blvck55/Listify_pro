...
<header style="position:fixed;top:0;inset-inline:0;z-index:50;
  ...
  <div class="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between gap-6">
    ...
    {{-- Logo --}}
    <a href="{{ route('welcome') }}" class="flex items-center gap-2.5 flex-shrink-0"
       style="text-decoration:none">
      <img src="{{ asset('images/logo.svg') }}" alt="Listify" class="w-7 h-7 block" />
    ...
    {{-- Right: blvck credit · theme toggle · auth --}}
      ...
      <div>
        ...
        <div class="au d5 flex items-center gap-6 flex-wrap">
          ...
          <div class="flex items-center gap-1.5 text-xs" style="color:var(--wp-faint)">
            <i class="fa-solid {{ $icon }} text-[10px]" style="color:#3B82F6"></i>
            <span>{{ $text }}</span>
          </div>
          @endforeach
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
      ...
      <div class="reveal">
        <div class="text-xl font-extrabold tracking-tight mb-1"
             style="color:var(--wp-text)">{{ $num }}</div>
        <div class="text-xs" style="color:var(--wp-faint)">{{ $label }}</div>
      </div>
      @endforeach
    </div>
  </div>
  ...
  <div class="max-w-6xl mx-auto px-6">
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div class="feat-card reveal">
        ...
        <h3 class="text-sm font-semibold mb-2" style="color:var(--wp-text)">{{ $title }}</h3>
        <p class="text-sm leading-relaxed" style="color:var(--wp-faint)">{{ $desc }}</p>
      </div>
      @endforeach
    </div>
      ...
      <div class="reveal">
        <div class="rounded-xl overflow-hidden"
          ...
          <div class="grid grid-cols-3" style="border-bottom:1px solid var(--wp-border)">
            @foreach([['23','Completed','#22C55E'],['8','Pending','#F59E0B'],['3','Overdue','#EF4444']] as [$n,$l,$c])
            <div class="px-4 py-4 text-center"
                 style="{{ !$loop->last ? 'border-right:1px solid var(--wp-border)' : '' }}">
              <div class="text-xl font-extrabold tracking-tight mb-0.5"
                   style="color:{{ $c }}">{{ $n }}</div>
              <div class="text-[10px]" style="color:var(--wp-faint)">{{ $l }}</div>
            </div>
            @endforeach
          </div>
          ...
          <div class="p-5">
            ...
            <div class="flex items-end gap-2" style="height:56px">
              @foreach([['Mon',3],['Tue',5],['Wed',4],['Thu',7],['Fri',6],['Sat',8],['Sun',5]] as [$day,$val])
              <div class="flex-1 flex flex-col items-center gap-1">
                <div class="w-full rounded-t" style="height:{{ ($val/8)*100 }}%;
                     background:{{ $val>=7 ? '#3B82F6' : 'var(--wp-border)' }};min-height:4px">
                </div>
                <span class="text-[8px]" style="color:var(--wp-line)">{{ $day }}</span>
              </div>
              @endforeach
            </div>
          </div>
          ...
          <div class="px-5 pb-5 space-y-2.5">
            ...
            @foreach([['High','30%','#EF4444'],['Medium','50%','#F59E0B'],['Low','20%','#22C55E']] as [$l,$p,$c])
            <div class="flex items-center gap-3">
              <span class="text-[10px] w-12 flex-shrink-0" style="color:var(--wp-faint)">{{ $l }}</span>
              ...
              <span class="text-[10px] w-7 text-right" style="color:var(--wp-line)">{{ $p }}</span>
            </div>
            @endforeach
          </div>
...
<footer class="py-8" style="border-top:1px solid var(--wp-border)">
  <div class="max-w-6xl mx-auto px-6
    ...
    <div class="flex items-center gap-2.5">
      <img src="{{ asset('images/logo.svg') }}" alt="Listify" class="w-5 h-5 block" />
