
<!doctype html>
<html lang="en" class="">
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1"/>
  <meta name="csrf-token" content="<?php echo e(csrf_token()); ?>">
  <title>Listify — <?php echo e($pageTitle ?? 'Dashboard'); ?></title>

  
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=Inter:wght@400;500;600&display=swap" rel="stylesheet">

  
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">

  <?php echo app('Illuminate\Foundation\Vite')(['resources/css/app.css', 'resources/js/app.js']); ?>
  <?php echo \Livewire\Mechanisms\FrontendAssets\FrontendAssets::styles(); ?>


  
  <script>
    (function() {
      if (localStorage.getItem('lf-theme') === 'dark' ||
          (!localStorage.getItem('lf-theme') && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
        document.documentElement.classList.add('dark');
      }
    })();
  </script>
</head>

<body style="background:var(--bg-page);color:var(--text-primary);font-family:var(--font-body);min-height:100vh;">


<nav class="lf-nav">
  <div class="lf-wrap lf-nav-inner">

    
    <a href="<?php echo e(Auth::user()->isAdmin() ? route('admin.dashboard') : route('dashboard')); ?>"
       class="lf-logo" style="align-items:center">
      <img src="<?php echo e(asset('images/logo.svg')); ?>" alt="Listify logo" class="lf-logo-img" style="width:32px;height:32px;display:block" />
      <span class="lf-logo-text">Listify</span>
    </a>

    
    <div class="lf-nav-tabs" style="position:absolute;left:50%;transform:translateX(-50%);">
      <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(Auth::user()->isAdmin()): ?>
        <a href="<?php echo e(route('admin.users')); ?>"
           class="lf-tab <?php echo e(request()->routeIs('admin.users') ? 'active' : ''); ?>">
          <i class="fa-solid fa-users fa-xs" style="margin-right:5px;opacity:.7"></i>Users
        </a>
        <a href="<?php echo e(route('admin.dashboard')); ?>"
           class="lf-tab <?php echo e(request()->routeIs('admin.dashboard') ? 'active' : ''); ?>">
          <i class="fa-solid fa-list-check fa-xs" style="margin-right:5px;opacity:.7"></i>Tasks
        </a>
        <a href="<?php echo e(route('admin.reports')); ?>"
           class="lf-tab <?php echo e(request()->routeIs('admin.reports') ? 'active' : ''); ?>">
          <i class="fa-solid fa-chart-bar fa-xs" style="margin-right:5px;opacity:.7"></i>Reports
        </a>
        <form method="POST" action="<?php echo e(route('logout')); ?>" style="display:inline">
          <?php echo csrf_field(); ?>
          <button class="lf-tab">
            <i class="fa-solid fa-right-from-bracket fa-xs" style="margin-right:5px;opacity:.7"></i>Logout
          </button>
        </form>
      <?php else: ?>
        <a href="<?php echo e(route('dashboard')); ?>"
           class="lf-tab <?php echo e(request()->routeIs('dashboard') ? 'active' : ''); ?>">
          <i class="fa-solid fa-house fa-xs" style="margin-right:5px;opacity:.7"></i>Dashboard
        </a>
        <a href="<?php echo e(route('tasks.history')); ?>"
           class="lf-tab <?php echo e(request()->routeIs('tasks.history') ? 'active' : ''); ?>">
          <i class="fa-solid fa-clock-rotate-left fa-xs" style="margin-right:5px;opacity:.7"></i>History
        </a>
        <form method="POST" action="<?php echo e(route('logout')); ?>" style="display:inline">
          <?php echo csrf_field(); ?>
          <button class="lf-tab">
            <i class="fa-solid fa-right-from-bracket fa-xs" style="margin-right:5px;opacity:.7"></i>Logout
          </button>
        </form>
      <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    </div>

    
    <div class="lf-nav-right">

      
      <button class="lf-theme-toggle" id="themeToggle" title="Toggle dark mode">
        <i class="fa-solid fa-moon" id="themeIcon"></i>
      </button>

      
      <?php
$__split = function ($name, $params = []) {
    return [$name, $params];
};
[$__name, $__params] = $__split('notification-bell', []);

$__key = null;

$__key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-229987574-0', $__key);

$__html = app('livewire')->mount($__name, $__params, $__key);

echo $__html;

unset($__html);
unset($__key);
unset($__name);
unset($__params);
unset($__split);
if (isset($__slots)) unset($__slots);
?>

      
      <div style="position:relative">
        <button class="lf-icon-btn" onclick="toggleDropdown('profileDrop')" title="Account">
          <i class="fa-regular fa-circle-user"></i>
        </button>
        <div class="lf-dropdown" id="profileDrop">
          <div class="lf-dropdown-header">
            <div style="font-size:14px;font-weight:700;color:var(--text-primary)"><?php echo e(Auth::user()->name); ?></div>
            <div style="font-size:12px;color:var(--text-muted)"><?php echo e(Auth::user()->email); ?></div>
            <span class="lf-badge-<?php echo e(Auth::user()->isAdmin() ? 'blue' : 'gray'); ?> lf-badge" style="margin-top:6px">
              <?php echo e(Auth::user()->role); ?>

            </span>
          </div>
          <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(Auth::user()->isAdmin()): ?>
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="lf-dropdown-item">
              <i class="fa-solid fa-gauge" style="font-size:14px"></i> Admin Panel
            </a>
          <?php else: ?>
            <a href="<?php echo e(route('dashboard')); ?>" class="lf-dropdown-item">
              <i class="fa-solid fa-house" style="font-size:14px"></i> Dashboard
            </a>
          <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
          <a href="<?php echo e(route('profile.show')); ?>" class="lf-dropdown-item">
            <i class="fa-solid fa-shield-halved" style="font-size:14px"></i> Security Settings
          </a>
          <form method="POST" action="<?php echo e(route('logout')); ?>">
            <?php echo csrf_field(); ?>
            <button type="submit" class="lf-dropdown-item danger" style="border-top:1px solid var(--border);width:100%">
              <i class="fa-solid fa-right-from-bracket" style="font-size:14px"></i> Sign out
            </button>
          </form>
        </div>
      </div>

    </div>
  </div>
</nav>


<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('success') || session('error')): ?>
  <div class="lf-wrap" style="margin-top:1rem" id="flashMessages">
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('success')): ?>
      <div class="lf-alert lf-alert-success" style="display:flex;justify-content:space-between;align-items:center">
        <span><i class="fa-solid fa-circle-check"></i> <?php echo e(session('success')); ?></span>
        <button onclick="this.closest('.lf-alert').style.opacity='0';setTimeout(()=>this.closest('.lf-alert').remove(),200)"
                style="background:none;border:none;cursor:pointer;color:inherit;opacity:.6;padding:0;line-height:1">
          <i class="fa-solid fa-xmark"></i>
        </button>
      </div>
    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('error')): ?>
      <div class="lf-alert lf-alert-error" style="display:flex;justify-content:space-between;align-items:center">
        <span><i class="fa-solid fa-triangle-exclamation"></i> <?php echo e(session('error')); ?></span>
        <button onclick="this.closest('.lf-alert').style.opacity='0';setTimeout(()=>this.closest('.lf-alert').remove(),200)"
                style="background:none;border:none;cursor:pointer;color:inherit;opacity:.6;padding:0;line-height:1">
          <i class="fa-solid fa-xmark"></i>
        </button>
      </div>
    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
  </div>
  <script>
    // Auto-dismiss flash messages after 4 seconds
    setTimeout(() => {
      const f = document.getElementById('flashMessages');
      if (f) { f.style.transition = 'opacity .4s'; f.style.opacity = '0'; setTimeout(() => f.remove(), 400); }
    }, 4000);
  </script>
<?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>


<main class="lf-wrap" style="padding-top:2rem;padding-bottom:3rem">
  <?php echo e($slot); ?>

</main>


<footer class="lf-footer">
  <div class="lf-wrap" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem">
    <div style="display:flex;align-items:center;gap:8px">
      <img src="<?php echo e(asset('images/logo.svg')); ?>" alt="Listify" style="width:24px;height:24px;display:block" />
      <span style="font-size:12px;color:var(--text-muted);font-weight:600">Listify</span>
    </div>
    <div style="display:flex;gap:1.5rem">
      <a href="<?php echo e(route('about.show')); ?>">About</a>
      <a href="<?php echo e(route('contact.show')); ?>">Contact</a>
      <a href="<?php echo e(route('terms.show')); ?>">Terms</a>
    </div>
  </div>
</footer>


<script>
// Dark mode
const html = document.documentElement;
const btn  = document.getElementById('themeToggle');
const icon = document.getElementById('themeIcon');

function applyTheme(dark) {
  html.classList.toggle('dark', dark);
  icon.className = dark ? 'fa-solid fa-sun' : 'fa-solid fa-moon';
}
applyTheme(html.classList.contains('dark'));

btn.addEventListener('click', () => {
  const isDark = !html.classList.contains('dark');
  applyTheme(isDark);
  localStorage.setItem('lf-theme', isDark ? 'dark' : 'light');
});

// Dropdowns
function toggleDropdown(id) {
  const el = document.getElementById(id);
  const open = el.classList.contains('open');
  document.querySelectorAll('.lf-dropdown').forEach(d => d.classList.remove('open'));
  if (!open) el.classList.add('open');
}
document.addEventListener('click', e => {
  if (!e.target.closest('.lf-icon-btn') && !e.target.closest('.lf-dropdown')) {
    document.querySelectorAll('.lf-dropdown').forEach(d => d.classList.remove('open'));
  }
});
</script>

<?php echo \Livewire\Mechanisms\FrontendAssets\FrontendAssets::scripts(); ?>

</body>
</html>
<?php /**PATH C:\Users\saran\listify\resources\views/layouts/app.blade.php ENDPATH**/ ?>